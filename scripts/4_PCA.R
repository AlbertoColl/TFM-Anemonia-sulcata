...
datos_pca <- scale(select(datos, SOD.pie, SOD.tent, CAT.pie, CAT.tent, GST.pie, GST.tent, MDA.pie, MDA.tent, TEAC.pie, TEAC.tent)) # Clorofila, peso, proteina y TEAC no son muy relevantes parece ser
...
datos_pca <- datos_pca[-c(4,9), ] # El punto 9 es potencial outlier pero parece que no influencia mucho el resultado del análisis. Repetir y evaluar de nuevo cuando esten todas las variables.
...
eigenvalues$PC <- factor(eigenvalues$PC, levels = c("PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10", "PC11"))
